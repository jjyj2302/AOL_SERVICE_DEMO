...
class ThreatHuntingCrew():
    ...
    # ========================================================================
    # Individual INDEPENDENT Agent Execution Methods
    # Each agent works INDEPENDENTLY without depending on other agents
    # ========================================================================

    def run_triage_only(self, ioc: str) -> dict:
        """
        Run ONLY Triage Specialist - completely independent

        Args:
            ioc: The indicator of compromise to investigate

        Returns:
            Triage assessment results (TriageOutput)
        """
        print(f"🔍 [TRIAGE ONLY] Starting independent triage for IOC: {ioc}")

        # Create standalone task without context dependencies
        triage_task = Task(
            config=self.tasks_config['initial_ioc_assessment'],
            agent=self.triage_specialist(),
            output_pydantic=TriageOutput,
            output_file='reports/triage_only.json'
        )
        ...
        result = solo_crew.kickoff(inputs={'ioc': ioc})
    ...
    def run_malware_only(self, ioc: str) -> dict:
        """
        Run ONLY Malware Specialist - completely independent

        Args:
            ioc: The indicator of compromise to investigate

        Returns:
            Malware analysis results (MalwareAnalysisOutput)
        """
        print(f"🔍 [MALWARE ONLY] Starting independent malware analysis for IOC: {ioc}")
        ...
        malware_task = Task(
            ...
            agent=self.malware_specialist(),
            output_pydantic=MalwareAnalysisOutput,
            output_file='reports/malware_only.json'
        )
    ...
    def run_infrastructure_only(self, ioc: str) -> dict:
        """
        Run ONLY Infrastructure Hunter - completely independent

        Args:
            ioc: The indicator of compromise to investigate

        Returns:
            Infrastructure correlation results (InfrastructureCorrelationOutput)
        """
        print(f"🔍 [INFRASTRUCTURE ONLY] Starting independent infrastructure analysis for IOC: {ioc}")
        ...
        infra_task = Task(
            ...
            agent=self.infrastructure_hunter(),
            output_pydantic=InfrastructureCorrelationOutput,
            output_file='reports/infrastructure_only.json'
        )

        # Single-agent crew
        solo_crew = Crew(
            agents=[self.infrastructure_hunter()],
            tasks=[infra_task],
            process=Process.sequential,
            verbose=True,
            memory=False
    ...
    def run_campaign_only(self, ioc: str) -> dict:
        """
        Run ONLY Campaign Analyst - completely independent

        Args:
            ioc: The indicator of compromise to investigate

        Returns:
            Campaign intelligence results (CampaignIntelligenceOutput)
        """
        print(f"🔍 [CAMPAIGN ONLY] Starting independent campaign analysis for IOC: {ioc}")
        ...
        campaign_task = Task(
            ...
            agent=self.campaign_analyst(),
            output_pydantic=CampaignIntelligenceOutput,
            output_file='reports/campaign_only.json'
        )

        # Single-agent crew
        solo_crew = Crew(
            agents=[self.campaign_analyst()],
            tasks=[campaign_task],
            process=Process.sequential,
            verbose=True,
            memory=False

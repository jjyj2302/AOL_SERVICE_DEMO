...
서버 시작 시 모든 API 키를 DB에서 로드하여 메모리에 캐싱.
...
class APIKeyCache:
    ...
    def load_all_keys(self, db: Session) -> None:
        """
        DB에서 모든 API 키 로드하여 캐시에 저장

        main.py의 lifespan에서 서버 시작 시 한 번만 호출.
        """
        ...
        for service_key, service_def in SERVICE_DEFINITIONS.items():
            required_keys = service_def.required_keys if service_def.required_keys else [service_key]
